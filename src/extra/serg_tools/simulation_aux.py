...
class ShStep:
    ...
    def Run(self, log):
        ...
        log.info("Returned %d" %exit_status)
        return exit_status
...
def GetPrestoSteps(params, run_params):
    presto_steps = [
        ShStep(["mkdir -p %s/presto &&" % run_params.data_path,
        ...
        ShStep(["cd %s/presto &&" % run_params.data_path,
                os.path.join(current_dir, "run_simple.sh"),
