...
namespace cdr_labeler {
    void CDRLabelingWriter::OutputCDRDetails() const {
        ...
        out << "Read_name\tCDR1_nucls\tCDR1_start\tCDR1_end\tCDR2_nucls\tCDR2_start\tCDR2_end\tCDR3_nucls\tCDR3_start\tCDR3_end" << std::endl;
        for(auto it = clone_set_.cbegin(); it != clone_set_.cend(); it++) {
            ...
            out << cdr_clone.Read().name << "\t" << cdr_clone.CDR1() << "\t" << cdr1_range.start_pos + 1 << "\t" <<
                    cdr1_range.end_pos + 1 << "\t" << cdr_clone.CDR2() << "\t" << cdr2_range.start_pos + 1 << "\t" <<
                    cdr2_range.end_pos + 1 << "\t" << cdr_clone.CDR3() << "\t" << cdr3_range.start_pos + 1 << "\t" <<
                    cdr3_range.end_pos + 1 << std::endl;
        }
        out.close();
