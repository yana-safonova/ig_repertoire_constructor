...
struct fasta_read {
	...
	fasta_read(string new_name, string new_seq) :
		name(new_name.substr(1, new_name.size() - 1)),
		seq(new_seq) { }
...
typedef FastaReader<fasta_read, FastaReadConstructor> SingleFastaReader;
typedef FastaReader<paired_fasta_read, PairedFastaReadConstructor> PairedFastaReader;
