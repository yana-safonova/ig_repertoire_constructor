...
namespace antevolo {
    ...
    void AntEvoloOutputWriter::OutputTreeStats() const {
        ...
        out << "Tree_id\tNum_vertices\tTree_depth\tRoot_depth\tNum_uniqie_SHMs\tNum_added_SHMs\tNum_synonymous_SHMs" << std::endl;
        size_t tree_index = 1;
        for(auto it = annotated_storage_.cbegin(); it != annotated_storage_.cend(); it++) {
            out << tree_index << "\t" << it->Tree().NumVertices() << "\t" << it->Tree().Depth() << "\t" <<
