...
def DrawHistogram(keys, histogram_setting):
    ...
    RemoveEmptyKeysFromHistogram(keys, histogram_setting)
    if len(keys) == 0:
...
def NumValuesGreaterThresholds(X, threshold):
    return len([x for x in X if x >= threshold])
...
def DrawPlot(x, y, setting):
    if setting.colors != "":
        pylab.plot(x, y, color = setting.colors, label = setting.label, marker = setting.marker, linestyle = setting.linestyle)
    else:            
        pylab.plot(x, y, label = setting.label, marker = setting.marker, linestyle = setting.linestyle) 
    if setting.xlog_scale:        
        pylab.gca().set_xscale("log")
    if setting.draw_legend:
        pylab.legend(loc = setting.legend_loc)
