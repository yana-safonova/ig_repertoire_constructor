...
void create_console_logger() {
    ...
    logger *lg = create_logger("report_pcr_error_rate");
    lg->add_writer(std::make_shared<console_writer>());
    attach_logger(lg);
}

void parse_options(int argc, const char* const* argv, std::string& input_file_path) {
...
int main(int argc, const char* const* argv) {
    ...
    parse_options(argc, argv, input_file_path);

    ErrorAnalyzer error_analyzer;
    error_analyzer.readData(input_file_path);
    error_analyzer.performAnalysis();
}