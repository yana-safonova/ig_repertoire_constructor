...
struct partial_order_graph {
    ...
    size_t sequences_count() const noexcept;
...
private:
    ...
    pair_vector select_xmatches(pair_vector const& transitions, size_t m) const;
    pair_vector align(std::vector<kmer> const& kmer_seq) const;

    void add_indel_region(std::vector<node*>& new_nodes, std::vector<kmer> const& kmer_seq,
