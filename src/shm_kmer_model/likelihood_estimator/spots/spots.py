...
def hotspots_indexes():
    kmer_names = kmer_utilities.kmer_names()
    return np.array([i for i in xrange(len(kmer_names)) if kmer_names[i] in hotspots()])

def coldspots_indexes():
    kmer_names = kmer_utilities.kmer_names()
    return np.array([i for i in xrange(len(kmer_names)) if kmer_names[i] in coldspots()])
