...
#include "hogc.hpp"
using hogc::hammingOverlapGraph;
...
bool parse_cmd_line_arguments(int argc, char **argv, HOGCParameters &args) {
    using std::cout;
    using std::cerr;
    using std::endl;
    ...
    config.add_options()
            ...
            ("overlap", po::value<int>(&args.overlap)->default_value(args.overlap),
            "minimum overlap size (TODO write about negative)")
    ...
    if (vm.count("version")) {
        cout << bformat("Hamming Overlap Graph Constructor, part of IgReC version %s; git version: %s") % build_info::version % build_info::git_hash7 << endl;
        return false;
    }
...
int main(int argc, char **argv) {
    ...
    INFO(bformat("Overlap graph construction using %d threads starts") % args.threads);
    INFO("Strategy " << args.strategy << " was chosen");

    size_t discarded_reads = discardedReads(input_reads, args.tau, args.k, args.strategy, args.overlap);
    if (discarded_reads) {
        WARN("Reads discarded: "<< discarded_reads);
    }

    size_t num_of_dist_computations;
    auto graph = hammingOverlapGraph(input_reads,
                                     args.tau,
                                     args.overlap,
                                     args.k,
                                     args.strategy,
