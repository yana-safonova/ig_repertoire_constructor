...
namespace vj_finder {
    void VJParallelProcessor::Initialize() {
        ...
        for(size_t i = 0; i < num_threads_; i++)
            info_per_thread.push_back(VJAlignmentInfo(new VJQueryProcessor(algorithm_params_, read_archive_, v_db_, j_db_)));
    }
...
#pragma omp parallel for schedule(dynamic)
        for(size_t i = 0; i < read_archive_.size(); i++) {
            ...
            VJQueryProcessor& vj_query_processor = *(info_per_thread[thread_id].vj_query_processor);
