...
class AssemblyStage {
  public:
    ...
    virtual void load(const std::string &load_from, const char* prefix = NULL);
    virtual void save(const std::string &save_to, const char* prefix = NULL) const;
    virtual void run(const char* started_from = NULL) = 0;
...
class StageManager {
  ...
  public:
    ...
    void run(const char* start_from = NULL);
